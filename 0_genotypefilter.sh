...
python ${SCRIPTS_DIR}/genocounts_groups_threeway.py ${WORKING_DIR}/lacnasgut_pops.txt ${PREFIX}.SNPs.fspi.rm.table > ${PREFIX}.SNPs.fspi.rm.counts.v2.txt

awk -v OFS='\t' '{print $1,$2,$3,$4,$5,$6,$9,$10,$13,$14}' \
    ${PREFIX}.SNPs.fspi.rm.counts.v2.txt \
    > ${PREFIX}.SNPs.fspi.rm.AIMS_counts.v2.txt

awk -v OFS='\t' 'NR>1 {print $1,$2,$3,$4,$5,$6,$9,$10,$13,$14}' \
    ${PREFIX}.SNPs.fspi.rm.counts.v2.txt \
    > ${PREFIX}.SNPs.fspi.rm.AIMs_counts.v2.txt

#remove site where all species have the alternative allele
...
python ${SCRIPTS_DIR}/thin_positions.py 100 ${PREFIX}.SNPs.fspi.rm.AIMs_counts.v2.filtered.txt ${PREFIX}.SNPS.thinned_coords.txt

#thin by coordinates established in thin_positions.py
awk 'NR==FNR {coords[$1":"$2]; next} ($1":"$2) in coords' \
    ${PREFIX}.SNPS.thinned_coords.txt \
    ${PREFIX}.SNPs.fspi.rm.AIMs_counts.v2.filtered.txt \
    > ${PREFIX}.SNPs.fspi.rm.AIMs_counts.v2.filtered.thinned.txt

#edited to remove underscore in chromosome
awk 'BEGIN{OFS="\t"} {gsub(/Chr_/,"Chr-",$1); print $1, $2, $3, $4}' \
    ${PREFIX}.SNPs.fspi.rm.AIMs_counts.v2.filtered.thinned.txt \
    > /project/dtataru/ancestryinfer/AIMs_panel15_final.AIMs.v2.txt

awk 'BEGIN{OFS="\t"} {gsub(/Chr_/,"Chr-",$1); print $1, $2, $5, $6, $7, $8, $9, $10}' \
    ${PREFIX}.SNPs.fspi.rm.AIMs_counts.v2.filtered.thinned.txt \
    > /project/dtataru/ancestryinfer/AIMs_panel15_final.AIMs_counts.v2.txt
