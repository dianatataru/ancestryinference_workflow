...
while((my $line1=<IN1>) && (my $line2=<IN2>) && (my $line3=<IN3>) && (my $line4=<IN4>) && (my $line5=<IN5>) && (my $line6=<IN6>)){
	...
	if($focal1 >= $thresh){$par1++; $total++;}
	if($focal2 >= $thresh){$par2++; $total++;}
	if($focal3 >= $thresh){$par3++; $total++;}
	if($focal4 >= $thresh){$par1=$par1+0.5; $par2=$par2+0.5; $par1par2++; $total++;}
	if($focal5 >= $thresh){$par1=$par1+0.5; $par3=$par3+0.5; $par1par3++; $total++;}
